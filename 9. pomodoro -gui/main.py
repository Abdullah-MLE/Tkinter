...
def scoreboard():
    global score, selected_break
    state_label.configure(text="Break")
    state_label.place(x=180, y=60)

    photo.config(file="break.png")
    canvas.create_image(200, 200, image=photo)
    time_label.configure(text=f"{selected_break:02}:{0:02}")
    if score == 0:
        ...
        score_label.configure(text="✔ ✔ ✔ ✔")
        state_label.configure(text="Long Break")
        state_label.place(x=80, y=60)
        photo.config(file="break.png")
        canvas.create_image(200, 200, image=photo)
        time_label.configure(text="20:00")
        score = 0
        selected_break = 20
...
def count_down(time):
    if time == -1:
        ...
        global score, selected_time
        if selected_time == 13 or selected_time == 25:
            break_button.configure(state="normal")
            selected_time = 0
            score += 1
            scoreboard()
        elif selected_time == 55:
            break_button.configure(state="normal")
            selected_time = 0
            score += 2
            scoreboard()
        elif selected_time == 80:
            break_button.configure(state="normal")
            selected_time = 0
            score += 4
            scoreboard()
...
def start_break():
    ...
    break_button.configure(state="disabled")
...
def set_brake(b_time):
    global selected_break
    selected_break = int(b_time[-2:])
